...
EntityId World::create_player(uint8_t player_index, Vec3Pos start_pos, bool /*team_b*/) {
    ...
    AABB aabb{
        { start_pos.x - FpPos::from_float(0.2f),
          start_pos.y,
          start_pos.z - FpPos::from_float(0.2f) },
        { start_pos.x + FpPos::from_float(0.2f),
          { start_pos.y.raw + FpPos::from_float(1.9f).raw },
          start_pos.z + FpPos::from_float(0.2f) }
    };
...
EntityId World::create_ball(Vec3Pos start_pos) {
    ...
    e.collider.material_id = MAT_DRY_GRASS;
    e.collider.sphere.radius = { BALL_RADIUS.raw >> 8 }; // Q15.16 -> Q24.8
...
void World::update_aabb_tree() {
    for (EntityId id = 0; id < MAX_ENTITIES; ++id) {
        ...
        if (e.collider.shape_type == ShapeType::SPHERE) {
            ...
            FpPos len = { (int32_t)(e.collider.capsule.local_tip.y.raw -
                                    e.collider.capsule.local_base.y.raw) };
            tight = {
                { pos.x - r, pos.y - r, pos.z - r },
                { pos.x + r, { pos.y.raw + len.raw + r.raw }, pos.z + r }
            };
        } else {
    ...
    for (auto& [eid, hit] : ccd_hits) {
        ...
        if (vn.raw < 0) {
            ...
            e.rigidbody.position = {
                { e.rigidbody.position.x.raw
                  + (hit.contact_point.x * hit.toi).raw >> 8 },
                { e.rigidbody.position.y.raw
                  + (hit.contact_point.y * hit.toi).raw >> 8 },
                { e.rigidbody.position.z.raw
                  + (hit.contact_point.z * hit.toi).raw >> 8 },
            };
        }
    }
}

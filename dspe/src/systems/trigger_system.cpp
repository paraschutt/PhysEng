...
void TriggerSystem::init_pitch_triggers(
    ...
    FpPos neg_half_len{-half_len.raw};
    ...
    make_trigger(TriggerEventType::GOAL_LEFT, AABB{
        Vec3Pos{neg_half_len - goal_d,  FpPos::zero(), -half_goal},
        Vec3Pos{neg_half_len,           goal_h,         half_goal}
    });

    // ── Right goal mouth (x = +half_len)
    make_trigger(TriggerEventType::GOAL_RIGHT, AABB{
        Vec3Pos{half_len,                          FpPos::zero(), -half_goal},
        Vec3Pos{FpPos{half_len.raw + goal_d.raw},  goal_h,         half_goal}
    });

    // ── Pitch boundary (inverted — fires when ball is OUTSIDE)
    make_trigger(TriggerEventType::OUT_OF_BOUNDS, AABB{
        Vec3Pos{neg_half_len, FpPos::zero(),              -half_width},
        Vec3Pos{half_len,     FpPos::from_float(30.0f),   half_width}
    }, /*inverted=*/true);

    // ── Left penalty box
    FpPos pa_len {layout.penalty_area_length.raw};
    FpPos pa_half{layout.penalty_area_width.raw >> 1};
    make_trigger(TriggerEventType::BALL_IN_BOX_LEFT, AABB{
        Vec3Pos{neg_half_len,                            FpPos::zero(),            -pa_half},
        Vec3Pos{FpPos{neg_half_len.raw + pa_len.raw},    FpPos::from_float(5.0f),   pa_half}
    });

    // ── Right penalty box
    make_trigger(TriggerEventType::BALL_IN_BOX_RIGHT, AABB{
        Vec3Pos{FpPos{half_len.raw - pa_len.raw},  FpPos::zero(),           -pa_half},
        Vec3Pos{half_len,                           FpPos::from_float(5.0f),  pa_half}
    });
    ...
    make_trigger(TriggerEventType::KICKOFF_INFRINGEMENT, AABB{
        Vec3Pos{neg_kr, FpPos::zero(),              neg_kr},
        Vec3Pos{kr,     FpPos::from_float(2.0f),    kr}
    });
}
